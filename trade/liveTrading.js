...
let cp;
//현재 보고있는 코인의 인덱스, 다른코인이 상폐돼도 간섭 받지 않기 위헤 만듦 1chori에서 1부터시작,원화가 없음
let nowCoin = 1;
...
coinStystems.forEach((a, i) => {
  if (i != 0) {
    ...
    setInterval(() => {
      ...
      // 코인의 현재가를 미국감으로 바꿔주는 구문.이를 통해 iff와 randomprice를 if문에서 폐지로 바꿔줄 수 있다.
...
const getRandomPrice = (coin, coinStystem, i) => {
  ...
  // 코인에 따른 변화폭 조절 조건문
  if (
    coinsInTradeJS[i].symbol == "DSC" ||
    coinsInTradeJS[i].symbol == "HGC" ||
    coinsInTradeJS[i].symbol == "GBC" ||
    coinsInTradeJS[i].symbol == "DGC"
  ) {
    if (iff[i] == 0) {
    ...
    iff[i] = randomPrice;
    // 일단 0이하면 상페를 시킴 보든 안보든 유저의 정보를 바꿈,코인의 정보를 바꿈
    if (randomPrice <= 0) {
      console.log(coin[i], "위쪽");
      coins[i] = "미국감";
      loginUser.coinVolume[coins[i]["symbol"]] = 0;
      let tempUser = JSON.parse(localStorage.getItem("userInformation")).map(
        (a) => {
 ...
 *********************************************************************/
    ...
    if (ten_chance == 10) {
      ...
      // 90%      500기준 300 ~ 700
      let half = cp / 2;
      // 밸런스패치, 올라갈 확률이 좀 더 많음
      randomPrice = Math.floor(cp + half + half / 8 - (Math.random() * cp + 1));
